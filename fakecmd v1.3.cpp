...
int main() {
	std::string command;
	SetConsoleTitle("ELEMENT PRELOADER v1 ---- Loading Fake CMD Server Framework...");
	printf("Please wait while the framework loader working.\n[-                             ] 1% done.");
	Sleep(300);
	system("cls");
	printf("Please wait while the framework loader working.\n[------                        ] 21% done.");
	Sleep(100);
		...
		if (command == "cmd-mode") {
			...
			for(;;) {
				...
				if(in == "continute") {
					system("cls");
					for(;;) {
						SetConsoleTitle("Fake CMD v1.3----CHATTING...");
						ask = "";
						printf("\nMessage > ");
						std::cin >> ask;
						if(ask == "hello") {
			...
			printf("Fake CMD version 1.3\nCopyright Flater Studio 2022\n");
		} else if(command == "update-what") {
			printf("What's new in this version of Fake CMD:\n1:New security system(PSN System).\n2:Added 1 new command, numbered 11, enter the command 'help' to view.\n"); 
